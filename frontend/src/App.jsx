...
function App() {
  ...
  // When device changes, get new stream
  useEffect(() => {
    ...
    navigator.mediaDevices.getUserMedia({ audio: { deviceId: selectedDeviceId } })
      ...
      .catch((err) => {
        alert("Could not access selected microphone: " + err.message);
      });
  }, [selectedDeviceId]);
  ...
  useEffect(() => {
    ...
    mic.onend = () => {
      if (isInterviewing) {
        const waitUntilSpoken = setInterval(() => {
          if (!window.speechSynthesis.speaking) {
            setIsProcessing(false);
            mic.start();
            clearInterval(waitUntilSpoken);
          }
        }, 100); 
      }
    };
    ...
    mic.onerror = (event) => {
      ...
      if (isInterviewing && event.error === "aborted") {
        setTimeout(() => mic.start(), 500);
      }
    };
  }, [isInterviewing, role, isProcessing]);
  ...
  const startInterview = async () => {
    ...
    // Get stream from selected device before starting recognition
    try {
    ...
    try {
      const res = await axios.post('http://localhost:8000/api/chat/', {
        message: `Start interview for role: ${role}`,
        role: role,
      });
  ...
  return (
    <>  
      <DarkVeil 
        ...
        resolutionScale={1}
      />
      <div className="heading-fixed">AI HR Interview Simulator</div>
          ...
          {!isInterviewing ? (
            <>
              <input
                ...
                placeholder="What role do you want to practice for? (e.g., Data Scientist)"
                value={role}
                onChange={(e) => setRole(e.target.value)}
                autoFocus
              />
              <div style={{ margin: "1em 0" }}>
                <label htmlFor="mic-select" style={{ fontWeight: 500, marginRight: 8 }}>🎤 Microphone:</label>
                <select
                  id="mic-select"
                  value={selectedDeviceId}
                  onChange={e => setSelectedDeviceId(e.target.value)}
                  style={{ borderRadius: 8, padding: "0.4em 0.8em" }}
                >
                  {devices.map(device => (
            ...
            <>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <button className="btn-danger" onClick={stopInterview}>
                  ⏹ Stop Interview
                </button>
                <div style={{ fontWeight: 500, color: '#647dee' }}>
